...
class App extends Component {
    ...
    MenuClick(obj) {
        ...
        !clickMenu.filter(item => item.path === json.path).length && clickMenuItem({
            type: 'clickMenu',
            value: json
        })
    }
    breadcrumbItem() {
        const { clickMenu } = this.props.state;
        return clickMenu.map(item => <Breadcrumb.Item index={item.path} onClick={this.historyPath} key={item.path}>
        ...
        return (
            <Fragment>
                <Layout className='container'>
                    <Sider style={{ background: '#fff' }} collapsible collapsed={this.state.collapsed} onCollapse={this.onCollapse.bind(this)}>
                        <Menu selectedKeys={currentPath} onClick={this.MenuClick.bind(this)}>
                            {this.subMenuItem()}
                        </Menu>
                    </Sider>
                    <Layout>
                        <div className='Header'>
                            <Breadcrumb >
                                <Breadcrumb.Item onClick={this.historyPath}><Link to='/'>index</Link></Breadcrumb.Item>
                                {this.breadcrumbItem()}
                            </Breadcrumb>
                        </div>
                        <Content>
